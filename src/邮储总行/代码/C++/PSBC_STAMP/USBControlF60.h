...
//data      --- 读取成功后保存读取的数据
//len       --- 数据长度(应大于17个字节)
USBCONTROLF60_API int ReadBlock(unsigned char block, char* data, unsigned char len);

//0x2A, 写指定块
//block     --- 块号
//data      --- 待写入数据
//len       --- 数据长度(最多支持写17个字节)
USBCONTROLF60_API int WriteBlock(unsigned char block, char* data, unsigned char len);
